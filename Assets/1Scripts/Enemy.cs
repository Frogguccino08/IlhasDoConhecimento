...
public class Enemy : MonoBehaviour
{
    ...
    public void Fraquezas(int id)
    {
        ...
        if (dano[id] > 0 && enemy.materialPlayer == 5 && (material[id] == 2 || material[id] == 0 && materialInimigo == 2))
        {
            if (enemy.currentCharge > 0)
            {
                enemy.currentCharge -= 1;
                enemy.controlConheci.SpawnConhecimento(enemy.maxCharge + enemy.ModCharge, enemy.currentCharge);
                StartCoroutine(list.AparecerPassiva(6, "Não degradável", "Você perdeu um de carga"));
            }
            else
            {
                float dano;
                ...
                StartCoroutine(list.AparecerPassiva(6, "Não degradável", "Você não tinha carga pra perder então recebeu dano"));
            }
        }
        //Papel -> Vidro (7 no efeito)
        if(dano[id] > 0 && enemy.materialPlayer == 3 && (material[id] == 1 || material[id] == 0 && materialInimigo == 1))
        {
            if(enemy.efeitosAtivos[1] > 0)
            {
                enemy.efeitosAtivos[1] -= 1;
